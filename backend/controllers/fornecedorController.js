...
exports.createFornecedor = async (req, res) => {
    const {cnpj, nome_fornecedor, telefone} = req.body;
    try {
        const result = await pool.query(
            'INSERT INTO fornecedores (cnpj, nome_fornecedor, telefone) VALUES ($1, $2, $3) RETURNING *' 
...
exports.updateFornecedor = async (req, res) => {
  const { id } = req.params;
  const { cnpj, nome_empresa, telefone } = req.body;
  try {
    const result = await pool.query(
      'UPDATE fornecedores SET cnpj = $1, nome_fornecedor = $2, telefone = $3 WHERE id = $4 RETURNING *',
      [cnpj, nome_fornecedor, telefone, id]
    );
